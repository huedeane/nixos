...
script -qfec "SUDO_PROMPT='NixOS' sudo -A nixos-rebuild switch --flake '$NIXOS_CONFIG#$NIXOS_HOST'" /dev/null < /dev/null 2>&1 \
...
if [ $exit_code -eq 0 ]; then
  ...
  if [ "$action" = "view" ]; then
    cat "$log_file" | fold -sw 130 | nl -ba -w2 -s"| " | rofi -dmenu -p "Build Output" -kb-accept-entry "" -kb-accept-custom "" -no-fixed-num-lines -theme "$HOME/.config/rofi/message-output.rasi"
  fi
done
