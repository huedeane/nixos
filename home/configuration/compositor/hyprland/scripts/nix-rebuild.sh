...
for s in nixos-build nixos-build-edit; do
  if tmux has-session -t "=$s" 2>/dev/null; then
    ...
    notify-send "NixOS" "A build is already running ($running)" \
      --urgency=critical \
...
status_file="$log_dir/${ts}.status"
fifo="$log_dir/${ts}.fifo"
id=$$
...
tmux new-session -d -s "$session" -x 220 -y 50 \
  "SUDO_PROMPT='NixOS' sudo -A nixos-rebuild switch --flake '$NIXOS_CONFIG#$NIXOS_HOST' 2>&1; echo \$? > '$status_file'"
...
tmux rename-window -t "$session" "nixos-rebuild"

mkfifo "$fifo"
notify-send "NixOS" "Building configuration..." \
  --urgency=normal \
  --expire-time=0 \
  --replace-id="$id" \
