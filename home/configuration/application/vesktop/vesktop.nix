    ...
    if editMode then
      {
        ...
        "vesktop/settings/settings.json".source =
          config.lib.file.mkOutOfStoreSymlink "${dirPath}/settings/settings.json";
        "vesktop/settings/quickCss.css".source =
          config.lib.file.mkOutOfStoreSymlink "${dirPath}/settings/quickCss.css";
        "vesktop/themes/system24-catppuccin-frappe.theme.css".source =
          config.lib.file.mkOutOfStoreSymlink "${dirPath}/themes/system24-catppuccin-frappe.theme.css";
      }
    else
      { };
}
